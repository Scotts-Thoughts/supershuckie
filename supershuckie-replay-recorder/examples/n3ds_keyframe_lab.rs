//! Keyframe codec experiments on a real Nintendo 3DS replay (format v8): decode every stored
//! keyframe state in order straight from the file, then for each consecutive pair try other ways
//! of storing the change. Research tool for `replay-3ds-format-research.md`.
//!
//! ```text
...
fn main() {
    ...
    if let Some(c) = csv.as_mut() {
        ...
        for n in ["base3", "nopre3", "undo3", "full3", "full1", "full6", "pages3", "hist3", "dedupe3", "full19"] { write!(c, ",{n},{n}_cms,{n}_dms").unwrap(); }
        writeln!(c).unwrap();
    }
    ...
    let mut history: std::collections::VecDeque<Vec<u8>> = std::collections::VecDeque::new();
    ...
    for (k, &(frame, level, state_len, uncompressed_len, frame_len, frame_offset)) in kfs.iter().enumerate() {
        ...
        if let (Some(prev), true) = (prev_state.as_ref(), in_range) {
            ...
            let mut hist_prefix: Vec<u8> = Vec::new();
            for h in history.iter() { hist_prefix.extend_from_slice(h); }
            hist_prefix.extend_from_slice(prev);

            let jobs: Vec<(&'static str, Box<dyn Fn() -> (usize, f64, f64) + Sync + '_>)> = vec![
                ("base3", Box::new(|| timed(&payload, &prev_payload, &Z { ..z3 }))),
                ("nopre3", Box::new(|| timed(&payload, &[], &Z { ..z3 }))),
                ("undo3", Box::new(|| { let mut u = prev_payload.clone(); u.extend_from_slice(&undo); timed(&payload, &u, &Z { window_log: 28, ..z3 }) })),
                ("full3", Box::new(|| timed(&payload, prev, &Z { window_log: 28, ..z3 }))),
                ("full1", Box::new(|| timed(&payload, prev, &Z { level: 1, window_log: 28, ..z3 }))),
                ("full6", Box::new(|| timed(&payload, prev, &Z { level: 6, window_log: 28, ..z3 }))),
                ("pages3", Box::new(|| timed(&page_payload, prev, &Z { window_log: 28, ..z3 }))),
                ("hist3", Box::new(|| timed(&payload, &hist_prefix, &Z { window_log: 29, ..z3 }))),
                ("dedupe3", Box::new(|| if new_bytes.is_empty() { (0, 0.0, 0.0) } else { timed(&new_bytes, prev, &Z { window_log: 28, ..z3 }) })),
                ("full19", Box::new(|| if heavy { timed(&payload, prev, &Z { level: 19, window_log: 28, ..z3 }) } else { (0, 0.0, 0.0) })),
            ];
            ...
            while history.iter().map(|h| h.len()).sum::<usize>() > 64 << 20 { history.pop_front(); }
            prev_payload = payload;
        }
        // Every page of every keyframe state goes into the global store.
        for p in state.chunks_exact(4096) { store.insert(page_hash(p)); }
