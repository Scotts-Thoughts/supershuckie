...
fn main() {
    ...
    // no CMake/corrosion or frame-server build.rs around to supply melonDS's static libraries
    // or the C++ runtime in that case). It is gated on the `link-cores` feature, which is only
    ...
    let core_libs: [(PathBuf, &str); 2] = [
        (build_dir.join("melonDS/src"), "core"),
        (build_dir.join("melonDS/src/teakra/src"), "teakra"),
    ];
    ...
    for (dir, lib) in &core_libs {
        ...
        // The `-bundle` modifier keeps these big, LTO'd archives out of the rlib; they are
        // resolved only at the final binary link, same as CMake's Qt link.
...
/// The C++ runtime and platform libraries melonDS's static archives call into. Emitted from
/// both melonds-rs/build.rs and mgba-rs/build.rs when `link-cores` is enabled; duplicate
...
fn link_runtime() {
    ...
    match target_os.as_str() {
        "windows" => {
            ...
            // melonds-rs's own rlib at melonds-rs's compile time, which places them on the
            // final link line ahead of `-lcore`/`-lteakra`. Since nothing has referenced
            // libstdc++'s symbols yet at that point, ld's single left-to-right pass pulls in
            // none of them and never revisits — producing undefined references to
            // std::filesystem/_Rb_tree/etc. `-bundle` keeps it an external `-lstdc++`
            // reference instead, ordered (like core/teakra) after the archives that need it.
        ...
        "macos" => {
            ...
            // melonDS's availability checks call ___isPlatformVersionAtLeast, which lives in
            // the compiler runtime archive clang links implicitly but rustc does not. Unlike
            // the frame server's build.rs (which links a binary directly and can use
            ...
            // the core archives above instead.
