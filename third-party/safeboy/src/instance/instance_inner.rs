...
pub trait RunnableInstanceFunctions {
    ...
    /// The bit this instance is presenting on its serial output while another instance clocks a
    /// transfer (this instance is the slave, external clock): the top bit of SB.
    ///
    /// `false` while serial is disabled, and `true` (with a log message) if this instance is the
    /// one clocking the transfer. Meant to be called from the master's `serial_transfer_bit_end`
    /// callback, before [`serial_set_data_bit`](Self::serial_set_data_bit).
    fn serial_get_data_bit(&self) -> bool;

    /// Shift `bit` into this instance's serial register from another instance clocking a transfer
    /// (this instance is the slave). Every eighth bit completes the byte: the transfer flag clears
    /// and the serial interrupt is requested. Ignored while serial is disabled or this instance
    /// is clocking the transfer itself.
    fn serial_set_data_bit(&mut self, bit: bool);

    /// Set the state of the infrared receiver (what another console's infrared LED shines on it).
    fn set_infrared_input(&mut self, on: bool);
...
impl RunnableInstanceFunctions for Gameboy {
    ...
    #[inline]
    fn serial_get_data_bit(&self) -> bool {
        self.inner.serial_get_data_bit()
    }

    #[inline]
    fn serial_set_data_bit(&mut self, bit: bool) {
        self.do_with_inner_mut(|inner| inner.serial_set_data_bit(bit))
    }

    #[inline]
    fn set_infrared_input(&mut self, on: bool) {
        self.do_with_inner_mut(|inner| inner.set_infrared_input(on))
    }
...
impl RunnableInstanceFunctions for RunningGameboy {
    ...
    #[inline]
    fn serial_get_data_bit(&self) -> bool {
        unsafe { GB_serial_get_data_bit(self.gb) }
    }

    #[inline]
    fn serial_set_data_bit(&mut self, bit: bool) {
        unsafe { GB_serial_set_data_bit(self.gb, bit) }
    }

    #[inline]
    fn set_infrared_input(&mut self, on: bool) {
        unsafe { GB_set_infrared_input(self.gb, on) }
    }
...
unsafe fn direct_access(gb: *mut GB_gameboy_t, access: DirectAccessRegion) -> DirectAccessDataMut<'static> {
    ...
    let data = unsafe {
        ...
        // A region the cartridge does not have (RAM on a ROM-only cartridge) comes back as a
        // null pointer with a size of zero; a slice must not be built from a null pointer.
