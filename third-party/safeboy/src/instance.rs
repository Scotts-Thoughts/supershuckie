...
impl Gameboy {
    ...
    /// A raw pointer to the pinned running instance, stable for as long as this `Gameboy` lives.
    ///
    /// For a callback of *another* instance that has to reach this one (a link cable between two
    /// instances: the master's `serial_transfer_bit_end` exchanges bits with the slave through
    /// [`RunnableInstanceFunctions::serial_get_data_bit`] and
    /// [`RunnableInstanceFunctions::serial_set_data_bit`]).
    ///
    /// # Safety
    ///
    /// Dereferencing it is only sound while this `Gameboy` is alive, is not being run or otherwise
    /// borrowed, and nothing else touches the instance through another path: the caller takes on
    /// the aliasing rules the `&mut self` methods normally enforce.
    pub fn running_instance_ptr(&mut self) -> *mut RunningGameboy {
        // SAFETY: the pointer is not used to move the pinned value.
        unsafe { self.inner.as_mut().get_unchecked_mut() as *mut RunningGameboy }
    }
}
